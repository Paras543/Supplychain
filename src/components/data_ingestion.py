import os 
import sys
from dataclasses import dataclass
import pandas as pd 
from sklearn.model_selection import train_test_split
from src.exception import CustomException
from src.logger import logging


@dataclass
class DataIngestConfig:
    raw_data_path :str = os.path.join('artifacts','raw.csv')
    train_data_path : str = os.path.join('artifacts','train.csv')
    test_data_path:str = os.path.join('artifacts','test.csv')


class DataIngestion:
    def __init__(self):
        self.ingestion_config = DataIngestConfig()

    def initiate_data_ingestion(self,source_path:str):
        logging.info('Started')
        try:
            df = pd.read_csv(source_path,encoding='latin1')
            logging.info('Read dataset with the shape')
            columns_to_drop = [
                
                'Customer Email', 'Customer Fname', 'Customer Id',
                'Customer Password', 'Customer Street', 'Customer Zipcode',
                'Order Zipcode',

                
                'Latitude', 'Longitude', 'Product Description', 'Product Image',
                'Product Card Id',

                
                'Delivery Status', 'Days for shipping (real)', 'shipping date (DateOrders)',
            ]
            df = df.drop(columns=columns_to_drop)

            logging.info('Dropped the columns')

            os.makedirs(os.path.dirname(self.ingestion_config.raw_data_path),exist_ok=True)
            df.to_csv(self.ingestion_config.raw_data_path,index=False)
            logging.info("We have saved the Cleaned data")

            train_set,test_set = train_test_split(df,test_size=0.2,random_state=42,stratify=df['Late_delivery_risk'])

            train_set.to_csv(self.ingestion_config.train_data_path,index=False)
            test_set.to_csv(self.ingestion_config.test_data_path,index=False)
            logging.info(
                f"Train shape: {train_set.shape}, Test shape: {test_set.shape}"
            )
            logging.info("Data ingestion completed successfully")

            return (
                self.ingestion_config.train_data_path,
                self.ingestion_config.test_data_path

            )
        except Exception as e:
            logging.error("Error occured during Ingestion")
            raise CustomException(e,sys)
        


            





